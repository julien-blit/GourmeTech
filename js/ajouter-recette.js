...
function copierRecette() {
  ...
  const ingredients = Array.from(document.querySelectorAll("#liste-ingredients li")).map(li => "- " + li.textContent).join("\n");
  const instructions = Array.from(document.querySelectorAll("#liste-instructions li")).map((li, i) => `${i + 1}. ${li.textContent}`).join("\n");
...
function telechargerPDF() {
  ...
  const image = document.getElementById("recette-image").src;
  ...
  doc.setFontSize(12);
  doc.text(`Catégorie: ${categorie}`, 20, 30);
  doc.text(`Temps: ${temps}`, 20, 40);
  doc.text(`Difficulté: ${difficulte}`, 20, 50);
  doc.addImage(image, "JPEG", 20, 60, 180, 100);
  doc.text("Ingrédients:", 20, 170);
  doc.text(ingredients, 20, 180);
